...
#
# Creates an ARM Cortex-M executable wired up with the appropriate QEMU startup code, linker script, and post-build .bin
# generation. The startup + syscalls helpers are compiled once and shared across all callers via OBJECT libraries.
# ---------------------------------------------------------------------------------------
...
function(add_arm_executable name)
    ...
    add_executable(${name} ${ARGN} $<TARGET_OBJECTS:cfiber_cortex_syscalls> $<TARGET_OBJECTS:cfiber_cortex_startup>)

    target_link_options(${name} PRIVATE -Wl,--gc-sections -specs=nano.specs -nostartfiles -T${_linker_script})

    add_custom_command(
        TARGET ${name} POST_BUILD COMMAND ${CMAKE_OBJCOPY} -O binary $<TARGET_FILE:${name}>
                                          $<TARGET_FILE_DIR:${name}>/${name}.bin COMMENT "Generating .bin from ELF")
endfunction()
